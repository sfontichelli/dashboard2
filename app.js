const STORAGE_KEY="aliafor-dashboard-pro-v3";
...
function statusTag(v){const cls=trafficClass(v);const label=cls==='bad'?'Rojo':cls==='warn'?'Amarillo':'Verde';return '<span class="tag '+cls+'">'+label+'</span>';}
...
function renderCards(){
  ...
  document.getElementById("cards").innerHTML=[
    ...
    {label:"% cumplimiento",value:totalPct.toFixed(1)+"%",sub:totalPct>100?"Por encima del budget":"Dentro del budget",cls:pctCls},
  ...
  document.getElementById("miniStats").innerHTML='<div class="mini"><div class="k">Diferencia</div><div class="v">'+((totalForecast-budget>=0?"+":"")+money(totalForecast-budget))+'</div></div><div class="mini"><div class="k">Proyección</div><div class="v">'+(budget?(((totalForecast/budget)-1)*100).toFixed(1):"0.0")+'%</div></div><div class="mini"><div class="k">Estado</div><div class="v">'+(trafficClass(totalPct)==='bad'?'Rojo':trafficClass(totalPct)==='warn'?'Amarillo':'Verde')+'</div></div>';
}
...
function renderTable(){
  ...
  Object.keys(g).forEach(area=>{
    ...
    html+='<tr class="area-row"><td class="sticky-left left-area">'+area+'</td><td class="sticky-left left-cat muted">Resumen del área</td>'+MONTHS.map(month=>tdMonth(areaMonths[month])).join("")+'<td class="right-ytd">'+money(areaYTD)+'</td><td class="right-budget">'+money(areaBudget)+'</td><td class="right-pct">'+statusTag(areaPct)+'</td><td class="right-forecast">'+money(areaForecast)+'</td></tr>';
    rows.forEach((r,idx)=>{
      const rowYTD=ytd(r.m);const rowPct=pct(rowYTD,r.budget);const rowForecast=forecast(r.m);
      html+='<tr><td class="sticky-left left-area">'+(idx===0?area:"")+'</td><td class="sticky-left left-cat">'+r.cat+'</td>'+MONTHS.map(month=>tdMonth(r.m[month])).join("")+'<td class="right-ytd">'+money(rowYTD)+'</td><td class="right-budget">'+money(r.budget)+'</td><td class="right-pct">'+rowPct.toFixed(1)+'%</td><td class="right-forecast">'+money(rowForecast)+'</td></tr>';
    });
    html+='<tr class="subtotal-row"><td class="sticky-left left-area">'+area+'</td><td class="sticky-left left-cat">Subtotal '+area+'</td>'+MONTHS.map(month=>tdMonth(areaMonths[month])).join("")+'<td class="right-ytd">'+money(areaYTD)+'</td><td class="right-budget">'+money(areaBudget)+'</td><td class="right-pct">'+areaPct.toFixed(1)+'%</td><td class="right-forecast">'+money(areaForecast)+'</td></tr><tr class="group-gap"><td colspan="18"></td></tr>';
  });
  ...
  html+='<tr class="total-row"><td class="sticky-left left-area">Total</td><td class="sticky-left left-cat">General</td>'+MONTHS.map(month=>tdMonth(totalMonths[month])).join("")+'<td class="right-ytd">'+money(totalYTD)+'</td><td class="right-budget">'+money(totalBudget)+'</td><td class="right-pct">'+totalPct.toFixed(1)+'%</td><td class="right-forecast">'+money(totalForecast)+'</td></tr>';
...
function renderMonthlyChart(){
  ...
  const width=680, height=250, left=42, right=16, top=14, bottom=34;
  ...
  [0,0.25,0.5,0.75,1].forEach(p=>{
    ...
    out+='<text class="chart-value" x="0" y="'+(y+4)+'">'+val+'M</text>';
  });
  totals.forEach((v,i)=>{
    const bw=step*0.62;
    ...
    out+='<text class="chart-label" x="'+(x+bw/2)+'" y="'+(height-10)+'" text-anchor="middle">'+MONTHS[i].split(' ')[0]+'</text>';
  });
  svg.innerHTML=out;
}
function exportCSV(){
