const STORAGE_KEY="aliafor-dashboard-pro-v2";
...
function renderTable(){
  ...
  Object.keys(g).forEach(area=>{
    ...
    html+=`<tr class="area-row"><td class="sticky-left left-area">${area}</td><td class="sticky-left left-cat muted">Resumen del área</td>${MONTHS.map(month=>tdMonth(areaMonths[month])).join("")}<td class="sticky-right right-ytd">${money(areaYTD)}</td><td class="sticky-right right-budget">${money(areaBudget)}</td><td class="sticky-right right-pct">${statusTag(areaPct)}</td><td class="sticky-right right-forecast">${money(areaForecast)}</td></tr>`;
    rows.forEach((r,idx)=>{
      const rowYTD=ytd(r.m);const rowPct=pct(rowYTD,r.budget);const rowForecast=forecast(r.m);
      html+=`<tr><td class="sticky-left left-area">${idx===0?area:""}</td><td class="sticky-left left-cat">${r.cat}</td>${MONTHS.map(month=>tdMonth(r.m[month])).join("")}<td class="sticky-right right-ytd">${money(rowYTD)}</td><td class="sticky-right right-budget">${money(r.budget)}</td><td class="sticky-right right-pct">${rowPct.toFixed(1)}%</td><td class="sticky-right right-forecast">${money(rowForecast)}</td></tr>`;
    });
    html+=`<tr class="subtotal-row"><td class="sticky-left left-area">${area}</td><td class="sticky-left left-cat">Subtotal ${area}</td>${MONTHS.map(month=>tdMonth(areaMonths[month])).join("")}<td class="sticky-right right-ytd">${money(areaYTD)}</td><td class="sticky-right right-budget">${money(areaBudget)}</td><td class="sticky-right right-pct">${areaPct.toFixed(1)}%</td><td class="sticky-right right-forecast">${money(areaForecast)}</td></tr><tr class="group-gap"><td colspan="18"></td></tr>`;
  });
  ...
  html+=`<tr class="total-row"><td class="sticky-left left-area">Total</td><td class="sticky-left left-cat">General</td>${MONTHS.map(month=>tdMonth(totalMonths[month])).join("")}<td class="sticky-right right-ytd">${money(totalYTD)}</td><td class="sticky-right right-budget">${money(totalBudget)}</td><td class="sticky-right right-pct">${totalPct.toFixed(1)}%</td><td class="sticky-right right-forecast">${money(totalForecast)}</td></tr>`;
  tbody.innerHTML=html;
  applyMonthVisibility();
}
function renderRankings(){
  const spend=[...DATA].map(r=>({name:r.cat,value:ytd(r.m)})).sort((a,b)=>b.value-a.value).slice(0,5);
  const deviation=[...DATA].map(r=>({name:r.cat,value:forecast(r.m)-r.budget})).sort((a,b)=>b.value-a.value).slice(0,5);
...
function exportCSV(){
  const header=["Área","Categoría",...MONTHS,"YTD","Budget","%","Forecast"];
  const rows=DATA.map(r=>{
    const yy=ytd(r.m);
    return [r.area,r.cat,...MONTHS.map(month=>r.m[month]),yy,r.budget,pct(yy,r.budget).toFixed(1)+"%",forecast(r.m)];
  });
