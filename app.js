const STORAGE_KEY="aliafor-dashboard-cloud-v6";
...
function forecastHtml(row){
  const y=ytd(row.m); const f=forecast(row);
  return '<span class="'+(y>=row.budget?'forecast-stop':'')+'">'+money(f)+'</span>';
}
...
function renderTable(){
  ...
  tbody.innerHTML=html; applyMonthVisibility();
}
...
async function saveInput(){
  ...
  if(row){
    ...
    if(api){
      try{
        await fetch(api,{method:"POST",body:JSON.stringify({mes:month,area:row.area,categoria:cat,monto:amount,observacion:note})});
        setCloudStatus("Google Sheets conectado");
      }catch(e){
        console.error(e); saveLocal(); setCloudStatus("Error de conexión");
      }
    }else{ saveLocal(); }
    renderAll();
  }
